...
export class PlanBuilder {
    ...
    parse(planLine: string, lineIndex: number | undefined): PlanStep | undefined {
        ...
        // eslint-disable-next-line @typescript-eslint/no-use-before-define
        const group = PddlPlanParser.planStepPattern.exec(planLine);
        if (group) {
...
export class PddlPlanParser {
    ...
    static parseOnePlan(planText: string, planPath: string, epsilon: number): Plan {
        const dummyDomain = new DomainInfo('uri', 1, 'domain', PddlSyntaxTree.EMPTY, new SimpleDocumentPositionResolver(''));
        const dummyProblem = new ProblemInfo('uri', 1, 'problem', 'domain', PddlSyntaxTree.EMPTY, new SimpleDocumentPositionResolver(''));
        const parser = new PddlPlanParser(dummyDomain, dummyProblem, { minimumPlansExpected: 1, epsilon: epsilon });
